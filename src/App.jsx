...
function StockModal({asin,t,onClose}){
  ...
  const chartData=(data?.history||[]).map(r=>{const d=new Date(r.date);return{date:r.date,label:MS2[d.getMonth()]+" "+d.getDate(),fba:r.fba,velocity:r.velocity}});
    ...
    <div style={{position:"relative",width:"min(900px,92vw)",maxHeight:"85vh",overflowY:"auto",background:t.card,borderRadius:16,border:"1px solid "+t.cardBorder,boxShadow:"0 24px 60px rgba(0,0,0,0.3)",padding:0}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"20px 24px 12px",borderBottom:"1px solid "+t.divider,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={{fontSize:18,fontWeight:800,color:t.text}}>{asin}</div>
          {data&&<div style={{fontSize:12,color:t.textSec,marginTop:4}}>{data.name}{data.shop&&<span style={{color:t.primary,fontWeight:600}}> · {data.shop}</span>}{data.sku&&<span style={{color:t.textMuted}}> · {data.sku}</span>}</div>}
        </div>
      ...
      <div style={{padding:"16px 24px"}}>
        ...
        :<>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10,marginBottom:20}}>
            {[{l:"FBA Stock",v:N(cur.fba)},{l:"Stock Value",v:$(cur.stockValue)},{l:"Velocity",v:(cur.velocity||0).toFixed(1)+"/day"},{l:"Days Left",v:cur.daysLeft||"—"},{l:"Reserved",v:N(cur.reserved)},{l:"Prep Stock",v:N(cur.prepStock)},{l:"ROI",v:(cur.roi||0)+"%"},{l:"Margin",v:(cur.margin||0).toFixed(1)+"%"}].map((k,i)=>
              <div key={i} style={{background:t.tableBg,borderRadius:8,padding:"10px 12px"}}>
                <div style={{fontSize:9,color:t.textMuted,textTransform:"uppercase",letterSpacing:1,fontWeight:600}}>{k.l}</div>
                <div style={{fontSize:16,fontWeight:700,color:t.text,marginTop:4}}>{k.v}</div>
              </div>
            )}
          </div>
          <div style={{fontSize:13,fontWeight:700,color:t.text,marginBottom:10}}>Stock History (12 months)</div>
          {chartData.length>0?<ResponsiveContainer width="100%" height={280}>
            <ComposedChart data={chartData}>
              ...
              <Tooltip content={({active,payload,label})=>active&&payload?.length?<div style={{background:t.card,border:"1px solid "+t.cardBorder,borderRadius:8,padding:"8px 12px",boxShadow:"0 4px 12px "+t.shadow}}>
                ...
                {payload.map((p,i)=><div key={i} style={{fontSize:11,color:p.color}}>{p.name}: {p.name==="FBA Stock"?N(p.value):p.value}</div>)}
              </div>:null}/>
...
function TeamPage({t,fSeller,fDaily,asinPlanBkData,onAsinClick}){
  ...
  return<div>
    ...
    {asinData.length>0&&<Sec title="ASIN Detail by Seller (A / P / Gap)" icon="" t={t}><div style={{borderRadius:12,border:"1px solid "+t.cardBorder,background:t.card,overflow:"hidden"}}><div style={{overflowX:"auto",maxHeight:520,overflowY:"auto"}}><table style={{width:"100%",borderCollapse:"separate",borderSpacing:0,fontSize:12.5}}><thead style={{position:"sticky",top:0,zIndex:2}}><tr>{THDASIN.map((h,i)=><th key={i} style={{padding:"11px 14px",textAlign:i<=2?"left":"right",color:h==="STOCK VALUE"?t.orange:h.includes("GP")?t.primary:t.textMuted,fontWeight:700,fontSize:10.5,textTransform:"uppercase",letterSpacing:.5,borderBottom:"2px solid "+t.divider,background:h==="STOCK VALUE"?t.tableBg:h.includes("GP")?t.primaryLight:t.tableBg,whiteSpace:"nowrap",minWidth:i<=2?70:100}}>{h}{h==="STOCK VALUE"&&<Tip text={TIPS.stockValue} t={t}/>}</th>)}</tr></thead><tbody>{asinData.map((r,i)=><tr key={i} onMouseEnter={e=>e.currentTarget.style.background=t.tableHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={sellerBreaks.has(i)?{borderTop:"2px solid "+t.divider}:{transition:"background .1s"}}><td style={{padding:"11px 14px",fontSize:13,fontWeight:600,letterSpacing:.3,borderBottom:"1px solid "+t.divider,color:t.textSec}}><AsinLink asin={r.a} onClick={onAsinClick||(()=>{})} t={t}/></td><td style={{padding:"11px 14px",fontWeight:600,borderBottom:"1px solid "+t.divider,fontSize:11.5,color:t.primary}}>{r.sl||"—"}</td><td style={{padding:"11px 14px",fontWeight:700,borderBottom:"1px solid "+t.divider}}>{r.br}</td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider,background:t.primaryGhost}}><APG actual={r.ga} plan={r.gp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><StockVal sv={r.sv} gp={r.ga} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ra} plan={r.rp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.aa} plan={r.ap} t={t} reverse/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ua} plan={r.up} t={t} isMoney={false}/></td></tr>)}</tbody></table></div><div style={{padding:"8px 14px",fontSize:10.5,color:t.textMuted,borderTop:"1px solid "+t.divider,background:t.card}}>{asinData.length} ASINs · Grouped by seller · Stock Value = estimated</div></div></Sec>}
